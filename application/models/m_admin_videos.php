...
class M_admin_videos extends CI_Model {

    function __construct(){
        parent::__construct();

        $this->main_table = "wc_videos";
        $this->locales_table = "wc_locales_videos";
    ...
    function getVideoDataFromYoutube($urlVideo){
        preg_match("/^.*((youtu.be\/)|(v\/)|(embed\/)|(watch\?))\??v?=?([^#\&\?]*).*/",$urlVideo,$matches);
        
        $idYoutube = $matches[6];
        
        $url_parse = "http://gdata.youtube.com/feeds/api/videos/".$idYoutube."?v=2&amp;alt=json-in-script&amp;callback=FeedCallback";
        
        $video_data = simplexml_load_file($url_parse);
